...
export default function reducer(
  ...
  switch (action.type) {
    ...
    case Actions.PASTE: {
      ...
      // This bit of code is to prevent pasting outside of the spreadsheet format
      const columnSize = state.model.data[0].length;

      if (active.column + copiedSize.columns > columnSize) {
        // TODO: Add an actual toast message here
        alert('You cannot paste outside of the spreadsheet format');
        return state;
      }
      // End of code to prevent pasting outside of the spreadsheet format
